...
**Verification:** README updated to reference only `infra/slack/slack-app-manifest.json`.
...
**Verification:** All debugging traces in archive, docs/misc/debugging/current/ removed if empty.
...
**Remove empty directories:** `docs/misc/debugging/current/`, `docs/plans/` if empty after moves.
...
**Check:**
- infra/docker/Dockerfile still references `scripts/setup-git-auth.sh` and `scripts/healthcheck.js` at root ✓
- Makefile still references `scripts/service.sh` ✓
- CI workflow unchanged ✓
- No source code references deleted files ✓
- README references `infra/slack/slack-app-manifest.json` only (not .yaml) ✓

...
- [ ] No infra/docker/Dockerfile, Makefile, or CI references are broken
...
### Out-of-Scope
- Moving scripts/service.sh, scripts/setup-git-auth.sh, scripts/healthcheck.js (infra/docker/Dockerfile/Makefile dependencies)
...
MOVE → docs/archive/debugging/:
  docs/misc/debugging/current/ask-user-q-duplicate-20260326/
  docs/misc/debugging/current/auto-resume-partial-failure-202603261550/
  docs/misc/debugging/current/image-error-mcp-path-20260326/
  docs/misc/debugging/current/image-processing-error-20260326/
  docs/misc/debugging/current/issue64-202603250015/
  docs/misc/debugging/current/token-rotation-race-20260326/
  docs/misc/debugging/current/uiask-buttons-remain-202603240912/
...
- infra/docker/Dockerfile references `scripts/setup-git-auth.sh` and `scripts/healthcheck.js` at root → NOT touched
- Makefile references `scripts/service.sh` → NOT touched
- CI workflow uses `npm ci` → compatible with bun.lock removal
- README references `infra/slack/slack-app-manifest.json` (or `.yaml`) → update to reference JSON only
...
| Delete bun.lock | tiny | Project uses npm exclusively (CI, infra/docker/Dockerfile, package.json). |
...
| Keep docs/misc/debugging/current/ directory (empty after move) | tiny | Remove empty dir. Future debugging traces go to docs/archive/debugging/. |

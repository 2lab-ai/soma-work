...
4. Existing `scripts/service.sh` install flow remains responsible for LaunchAgent regeneration and restart.
...
| Operator setup | `scripts/setup/*`, `scripts/new-deploy-setup.sh`, `docs/runbook/add-new-deploy.md` | interactive/manual setup | Ensure branch defaults and docs match `dev` naming |
...
- `scripts/service.sh` restarts the correct environment-specific LaunchAgent after deploy
...
→ Proceed with Vertical Trace via `docs/archive/features/main-deploy-migration/trace.md`

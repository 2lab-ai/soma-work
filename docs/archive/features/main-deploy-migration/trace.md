...
> Spec: docs/archive/features/main-deploy-migration/spec.md
...
- Transformation rules:
  ...
  - extracted `dist/`, `node_modules/`, `package.json`, `scripts/service.sh` → rsync to `TARGET`
...
- Existing flow remains:
  ...
  - `scripts/service.sh main install` regenerates/restarts LaunchAgent
...
| `scripts/service.sh main install` fails | main service not restarted | workflow failure |
...
- Files: `scripts/setup/05-system-prompt.sh`, `scripts/setup/07-deploy-dirs.sh`, `scripts/setup/09-github-environments.sh`, `scripts/new-deploy-setup.sh`, `docs/runbook/add-new-deploy.md`

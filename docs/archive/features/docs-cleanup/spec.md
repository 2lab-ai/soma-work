...
- [x] docs/current/spec/ 시리즈 (00~13)의 outdated 참조가 현행 코드에 맞게 수정됨
- [x] docs/current/spec/10-commands.md에 2026-03 기준 전체 커맨드 목록이 반영됨
- [x] docs/misc/reference/architecture.md의 LOC 수치와 모듈 목록이 현재 코드에 맞게 갱신됨
...
### In-Scope
- docs/current/spec/ 시리즈 현행화 (경로, 소스 파일, 인터페이스, 커맨드 목록)
- docs/misc/reference/architecture.md 갱신 (LOC, 모듈 구조)
...
- docs/plans/, docs/misc/research/ 정리 — 날짜 기반 참고 자료로 유지
- 코드 변경 (문서만 변경)
...
| Update | `docs/current/spec/03-session-management.md` | source 파일 참조 수정 |
| Update | `docs/current/spec/07-permission-system.md` | 프로젝트명 수정 |
| Update | `docs/current/spec/08-user-settings.md` | `accepted` 필드 추가 |
| Update | `docs/current/spec/09-configuration.md` | 경로 현행화 |
| Update | `docs/current/spec/10-commands.md` | 전체 커맨드 목록 갱신 |
| Update | `docs/current/spec/13-slack-ui-action-panel.md` | 파일 참조 수정 |
| Update | `docs/misc/reference/architecture.md` | LOC + 모듈 목록 갱신 |
...
| docs/plans/, docs/misc/research/는 유지 | tiny | 날짜 prefix로 이미 정리됨, 이동 불필요 |
| STV feature docs는 건드리지 않음 | small | 별도 라이프사이클, 간섭하면 혼란 |
| docs/misc/issues/current/는 유지 | tiny | 아직 open 상태일 수 있음 |

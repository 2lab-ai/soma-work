...
- **Manifest updates**:
  ...
  - `oauth_config.scopes.bot` already contains `assistant:write`; no scope
    change in this PR.
- **Rollback snapshot**: `docs/archive/manifests/slack-app-manifest.pre-666.json` captures the
...
2. Pre-merge verification: confirm `docs/archive/manifests/slack-app-manifest.pre-666.json` equals
...
4. **Dev workspace** (admin-operated):
   ...
   b. Save the current manifest as a backup (match against
      `docs/archive/manifests/slack-app-manifest.pre-666.json` for cross-check).
   c. Apply the new manifest from `infra/slack/slack-app-manifest.json` (includes
...
2. Re-apply `docs/archive/manifests/slack-app-manifest.pre-666.json` on the Slack app manifest
...
`docs/archive/manifests/slack-app-manifest.prev.json` (unrelated legacy rollback for slash-command
changes) is **not** the correct snapshot for this PR — use
`docs/archive/manifests/slack-app-manifest.pre-666.json`.

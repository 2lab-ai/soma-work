...
| deploy workflow는 `scripts/service.sh`와 target-side `npm ci --omit=dev --no-audit --no-fund`도 배포 계약 일부 | `.github/workflows/deploy.yml` deploy step |
| facade 분해가 이미 진행 중 (SlackHandler ~600 LOC, ClaudeHandler ~610 LOC, McpManager) | `docs/misc/reference/architecture.md` |
| MCP 분리 작업이 3 갈래로 동시 진행 중 | `docs/current/plans/mcp-refactor/`, `docs/current/plans/mcp-server-independence/`, `docs/current/plans/mcp-extraction/` |
...
- 입력: PR #871 v2 docs + current repo files (`package.json`, `tsconfig.json`, `src/mcp-config-builder.ts`, `.github/workflows/deploy.yml`, `scripts/service.sh`, `mcp-servers/*`, `somalib/*`).
- 추가 반영:
...
| 6 | bundle staging 예시는 `scripts/service.sh`/bootstrap script contract와 rsync filter-order risk를 충분히 다루지 않음 | plan.md §5 |

...
- *Spin* — injecting a continuation while a turn is live makes `handleConcurrency` **supersede (abort)** it, killing the model mid-work → empty output → tight eval loop (PROJ-4695). Guarded by the injection-time `isRequestActive` check (never supersede).
...
- Dispatch / parse / timeout failure → `pendingEval=undefined`, status preserved, `evalAttemptCount` **not** bumped (infra flakes aren't completed eval attempts). The loop then **self-heals**: while the consecutive-failure streak (`evalDispatchRetryCount`) is below `GOAL_EVAL_DISPATCH_MAX_RETRIES` (3), a re-eval is scheduled after the failure's advertised rate-limit window (gateway "retry in Ns", bounded to [1s, 1h]) or a 60s default backoff, and Slack posts `⏳ Goal completion evaluation hit a transient API error — retrying in Ns (attempt k/3)`. While the window is open, `evalBackoffUntil` gates `runOnce` so ordinary settles (e.g. a duplicate trigger queued behind the failing run) cannot re-dispatch against the same rate limit and burn the budget; the scheduled retry lifts the gate when it fires, a gated settle re-arms a (per-session deduped) retry for the remaining window — so a restart that loses the timer self-heals on the first post-restart settle — and the timestamp self-expires as a last resort. The scheduled retry re-enters through `onTurnSettled` after revalidating the failure-time `goalId`+`epoch` snapshot, so it re-checks every gate (idle / requestActive / pendingEval) at fire time and no-ops if the user weighed in during the wait; if the epoch moved during the failed eval itself, the lease is released and no retry (or notice) happens at all. A successful eval dispatch or any real user message resets the streak. Only when the budget is exhausted does Slack post the terminal `⚠️ Goal completion evaluation failed` with instructions to use `goal done` / `goal pause` / `goal clear` (the streak is reset there too, so a later user nudge starts a fresh budget). To make the advertised window visible through the eval timeout, the timeout aborts the dispatch first and only fails with the generic timeout error after a short grace window — the dispatch seam's abort rejection carries `poolRateLimitRetryMs` when the abort interrupted a rate-limit wait.

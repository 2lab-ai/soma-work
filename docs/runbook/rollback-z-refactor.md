...
**사전 참고**
- 관련 스펙: `docs/current/spec/z-command-master-spec.md` §12 (Rollback)
- 관련 이슈: #506 (/z 리팩터), #505 (tombstone), #507 (Phase 2 UI)
- 스냅샷 파일: `docs/archive/manifests/slack-app-manifest.prev.json` (pre-#506 상태)
...
# 3. 복원된 infra/slack/slack-app-manifest.json을 Slack API 콘솔에 업로드
#    https://api.slack.com/apps → (앱 선택) → App Manifest → 붙여넣기 → Save
#    Dev 앱과 Prod 앱 양쪽 모두 수행.

# 4. 롤백 결과를 커밋
git add infra/slack/slack-app-manifest.json
...
3. **Tier 3** 해제: `infra/slack/slack-app-manifest.json`을 `feat/z-phase1` 상태로 복구하고 Slack UI 업로드.

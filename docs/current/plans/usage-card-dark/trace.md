...
Spec: `docs/current/plans/usage-card-dark/spec.md`
...
Initial creation from `docs/current/plans/usage-card-dark/spec.md` (main approved via UIAskUserQuestion `approve_as_is`). 15 scenarios across carousel rendering, tab cache, streak metrics, dark palette, partial-empty fallback, action handler, and regression.

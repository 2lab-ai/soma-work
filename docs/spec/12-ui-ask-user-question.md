...
> 참고: 예전에 권장되던 라벨 내부 `(Recommended · N/M)` 패턴은 **deprecated** 입니다. 추천 옵션은 `recommendedChoiceId` 필드로 지정하세요. 렌더러가 해당 옵션을 별도 행 + `⭐ Recommended` 배너 + primary 스타일로 강조합니다(Slack · 대시보드 공통). 라벨 문자열에는 Recommended 표기를 더 이상 넣지 않습니다.

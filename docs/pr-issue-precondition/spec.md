...
> with `sourceIssueUrl=https://yourcompany.atlassian.net/browse/PROJ-4217` falling through to
...
```
Recognized: `https://yourcompany.atlassian.net/browse/PROJ-4217`.
Explicitly NOT recognized → `malformed-source-issue-url`:
- self-hosted (`https://jira.example.com/jira/browse/PROJ-4217`)
- board views with query-param ticket reference (`...?selectedIssue=PROJ-4217`)
- lowercase keys (`/browse/PROJ-4217`)
- keys with a leading digit (`4PROJ-4217`)

**Body marker** — case-insensitive `Closes <KEY>` (parallel to GitHub `Closes #N`), OR the
matching Jira URL anywhere in the body. Bare key mentions (`Related: PROJ-4217`, no `Closes`
prefix and no URL) **do not** satisfy — accidental commentary references would false-pass.
Comparison is upper-cased so `closes PROJ-4217` matches `PROJ-4217`. The URL-evidence path
...
- Accepting bare keys (`PROJ-4217` anywhere). Too loose; commentary like `Related: PROJ-4217`

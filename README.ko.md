...
| Jira Create PR | Jira 이슈 + "PR" | 자동 PR 생성 |
| Deploy | 배포 관련 요청 | 배포 워크플로우 |
| Onboarding | 신규 유저 / `onboarding` | 온보딩 가이드 |
...
│   ├── actions/                # 인터랙티브 액션 핸들러 (9 handlers)
│   ├── pipeline/               # 스트림 처리 파이프라인 (5 files)
│   ├── commands/               # 명령어 핸들러 (20 handlers)
...
├── plugin/                     # 플러그인 시스템 (마켓플레이스, 캐시, 매니저)
├── prompt/                     # 시스템 프롬프트
│   └── workflows/              # 워크플로우 프롬프트 (9 workflows)
...
docs/                           # 아키텍처 + 스펙 문서 (14 specs)
scripts/                        # 유틸리티 스크립트
```

| 항목 | 수치 |
|------|------|
| 소스 (테스트/로컬 제외) | 122 files, ~27,000 LOC |
| 테스트 | 43 files, ~11,100 LOC |
| 페르소나 | 12 files, ~4,700 LOC |
| 프롬프트/워크플로우 | ~2,150 LOC |

## Design Decisions

1. **Facade Pattern** - 복잡한 서브시스템을 3개의 Facade로 단순화
2. **Single Responsibility** - 파일당 하나의 책임 (122개 모듈)
3. **Pipeline Architecture** - 입력 전처리 → 세션 초기화 → 스트림 실행
4. **Workflow Dispatch** - 입력 분류 → 전문 워크플로우 프롬프트 적용 (9 workflows)
...
43개 테스트 파일(~11,100 LOC)이 핵심 경로를 커버합니다: 이벤트 라우팅, 스트림 처리, 명령어 파싱, 권한 검증, 도구 포맷팅, 세션 관리, 액션 핸들러, 파이프라인 처리 등.

...
| **zwork** | `z` + 태스크 / 이슈 / PR 링크 | z-파이프라인 구현 워크플로우 |
| **Default** | 기타 모든 입력 | 범용 코딩 어시스턴트 |

세션 핸드오프 전용 entrypoint(`z-plan-to-work`, `z-epic-update`)도 있습니다 (#695). Source of truth: [`src/prompt/workflows/`](./src/prompt/workflows/), [`src/dispatch-service.ts`](./src/dispatch-service.ts)의 `VALID_WORKFLOWS`.

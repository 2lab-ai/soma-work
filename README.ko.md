...
- `auth`(= `/z auth`) **용량 오버뷰**: llmux 모드에서 `auth` 카드는 admin 포함 모두에게 **읽기 전용 용량 오버뷰**가 기본이다. Claude → Codex → Grok 순 그룹화(그 외 그룹은 알파벳순, 그룹 안은 스케줄러 선택 순서); 그룹 합계는 5h/7d(및 `7d-fable` 등 scoped weekly) 잔여 비율을 **정규화된 계정분**(계정 1개 100% = 1, 플랜 가중치 없음)으로 합산 — llmux status 계약에는 utilization 비율과 리셋 시각만 있고 토큰 분모가 없으므로 토큰 잔고를 표시하거나 암시하지 않는다(누적 토큰은 소비량으로만 표기); 리셋은 Slack 로컬 절대 시각 + 남은 시간으로 표시하고 다음 리셋의 창·계정·회복 %p를 예고(추가 사용이 없을 때만 유효); 미제공·만료·범위 밖 값은 0%/100%가 아니라 "미제공"으로 표시, scoped 창은 공통 한도와 분리, cold/null 값은 합계에서 제외하고 측정 커버리지(n/m)를 표기; Grok/API-key는 llmux가 원시 요청·토큰 카운트를 비율로 접고 리셋 없는 버스트 한도에 합성 단기 리셋을 붙이는데, 카드는 그 합성 리셋을 실제 쿼터 창 카운트다운처럼 보여주지 않고 억제한다; Codex 수동 리셋 카운터(보유/현재 적용 가능)는 읽기 전용 표시(자동 사용 없음); 관리 컨트롤(모드 전환·설정·추가·삭제·계정 전환)은 [어드민 모드] 버튼 **명시 클릭**으로만 열리고 위조/스테일 admin 클릭은 서버에서 readonly로 강등; 큰 풀은 고정 페이지 크기로 페이지네이션되어 모든 계정에 도달 가능하며 Refresh/페이징은 현재 보기와 페이지를 유지한다. 레거시 CCT를 `auth` 안에 표시할 때도 페이지를 나누며, 변경 버튼과 모달 제출 후 같은 auth 보기로 돌아온다. 별도 `cct` 명령은 기존 동작을 유지한다. 상세: [README.md의 Auth capacity overview](./README.md#auth-capacity-overview) · 스펙/업스트림 검증: [`.prd/06-auth-capacity-spec.md`](./.prd/06-auth-capacity-spec.md)
- 메시지 명령 `autocompact`(메시지 맨 앞에 `/`를 붙인 `/autocompact`도 같은 파서로 허용; 별도 등록된 Slack slash command는 아님): **세션 스코프** auto-compact 토큰 임계값. 인자 없이 호출하면 현재 유효값과 출처를 보여주고, `800k`/`800K`/`0.8M`/`800000`/bare `800`(천 단위로 해석)으로 설정하며, `reset`으로 모델 기본값으로 되돌린다 — 세션에 저장되어 재시작에도 유지. 우선순위(높은 게 이김): 세션 오버라이드 → 모델 기본값(`fable`/`opus`/`sol[1m]` 1M 프로파일 750k·750k·600k, bare `gpt-5.6-sol` 340k, `astra`/`gpt-6`/`gpt6` 및 `gpt-6-astra[1m]` 1M 프로파일 600k, bare 리터럴 `gpt-6-astra`는 272k 컨텍스트에 240k, `grok-4.6` 450k, bare `claude-opus-5`는 기본값 없이 레거시 퍼센트로 폴백) → 레거시 유저 퍼센트(현재 모델 컨텍스트 윈도우 기준 환산). `grok-4.6[1m]`은 실재하지 않는 모델이라 항상 `use grok-4.6`으로 명시 거부된다. `/compact-threshold <percent>`는 **deprecated** 호환 어댑터 — 현재 모델 기준으로 퍼센트를 환산해 `/autocompact`와 같은 세션 토큰 슬롯에 쓰고, 변환 성공 시 옛 유저 전역 퍼센트 저장값을 지운다; 모델의 안전 최대치를 넘는 변환은 조용히 clamp하지 않고 명시적으로 실패한다. 자동 compact는 harness의 turn-end 체커가 유일한 트리거이며, SDK 자체의 native autocompact는 모든 해석된 모델 프로파일에서 꺼져 있다. 모델별 정확한 기본값 표는 [README.md의 Auto-compact thresholds 섹션](./README.md#auto-compact-thresholds-autocompact--compact-threshold)을 참고하세요.
- 모든 `gpt-…[1m]` 프로파일은 앞으로 추가되는 지원 모델까지 Sol과 같은 1M 윈도우, SDK 차단선 977k, 자동 압축 600k를 적용한다. 모델별 임계값을 따로 등록할 필요가 없다. 지원하지 않는 모델 변형을 생성하거나 명시적으로 선택한 bare 모델 ID를 바꾸지는 않으며, 세션 오버라이드는 계속 우선한다.
- 제공자가 compact 요청을 거절하면 실패 안내에 인증정보를 가린 오류 상세를 표시한다. Slack 표시 길이를 넘으면 생략 사실을 알리며, 서버 로그에는 가림 처리한 전체 원인을 남긴다. 실패 안내는 압축 성공을 뜻하지 않는다.

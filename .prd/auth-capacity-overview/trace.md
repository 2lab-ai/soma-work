...
- **G5 (test coverage gaps — flagged, no certification of behavior)** — updated for pass 2:
  ...
  - STILL OPEN: llmux Settings view-submission (`auth_llmux_settings`, auth/actions.ts:337–367) — URL-validation `response_action:'errors'` branch, reachable/⚠️ banner, `setLlmuxSettings` persistence call unpinned. llmux Remove view-submission (:402–423) unpinned (Add is pinned). `AuthHandler` text arms `auth llmux|cct` deny+apply and `auth switch` unpinned (status arm now covered; `key` covered in `auth-handler.key.test.ts`). `z_setting_auth_cancel` dismissal still relies on the shared z-settings route without an auth-specific assertion.

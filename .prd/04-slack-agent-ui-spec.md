...
| A37 | 컴팩트 레이아웃 (유저 2026-09-17 "좀더 컴팩트하게") | 헤더+상태 ≤4블록. Queue는 context 1줄 + 항목당 section 1블록(`n. 원문 · 상태`) + 우측 overflow 메뉴(Send now / Cancel / Retry / Resume). 페이지 nav는 2페이지 이상일 때만. **A39 이후**: 결합 패널은 더 이상 Queue를 싣지 않는다 — 이 Queue 레이아웃 규정은 패널이 호출하지 않는 `buildFollowupQueueBlocks`에만 남아 있고, 유저가 실제로 보는 큐 행은 A39의 항목 메시지다 |
...
| A39 | 큐 항목 인라인 표시 (유저 2026-09-17 "thread안에 유저가 메세지 쳤을때마다 출력해줘") | 메시지가 큐에 들어가면(스티어링 성공 여부와 무관) 같은 스레드에 항목 메시지 1건을 게시한다: `n. 원문(≤80자) · 상태` + 버튼(기존 action id·에포크 페이로드). 버튼은 **상태가 정한다**: `queued`/`steered` → `Send now`+`Cancel`, `paused` → `Resume`+`Cancel`, `uncertain` → `Retry`(확인 다이얼로그)+`Cancel`, `failed` → `Retry`+`Cancel`. `turnEpoch`는 `Send now`에만 실린다. freeze가 **park한 항목**(`paused`/`uncertain`)이면 freeze 안내문을 항목 줄 앞에 붙인다(패널 freeze 줄의 대체). 항목이 큐 안에서 상태만 바뀌면(스윕으로 `steered`→`queued`, Resume/Retry 결과) 그 메시지를 `chat.update`로 **다시 렌더**한다. 결합 패널에는 Queue 섹션을 렌더하지 않는다(헤더 breakdown의 `queued N`도 제거). 항목 메시지는 봇 게시이므로 패널 재앵커 알림을 발생시킨다. |
| A40 | 큐 확인 명령 (유저 2026-09-17 "따로 명령어 추가해주고 메세지 큐확인할수 있게") | `queue` / `큐` 명령은 그 스레드 세션의 미처리 항목(queued/steered/paused/uncertain/failed)만 나열한다. 행 **1개당 메시지 1건**(각각 A39 컨트롤 포함, 각각 자기 ts로 등록 — 공유 ts면 A41이 한 항목을 지울 때 나머지 행의 버튼까지 사라진다). 최대 10행이고 더 있으면 마지막에 `…외 N건` 메시지 1건(컨트롤 없음). 처리된 항목(resolved/cancelled)은 목록에 없다. 빈 큐면 `대기 중인 메시지가 없습니다` 1줄. 실행 중 턴을 끊지 않는 컨트롤 명령이다. |
| A41 | 처리된 항목 메시지 삭제 (유저 2026-09-17 "처리된 메세지는 삭제해줘") | 항목이 **resolved(consumed 포함)/cancelled**가 되면 A39·A40이 게시한 그 항목의 메시지를 `chat.delete`로 지운다(베스트 에포트, 실패는 로그). **`failed`는 지우지 않는다** — 아직 유저의 메시지이고 `Retry`로 되살릴 수 있으므로 메시지가 남아야 한다(A39의 `failed` 컨트롤이 그 메시지에 있다). 세션 삭제/고아 세션 정리가 미처리 항목을 일괄 `cancelled`로 만들 때도 같은 규칙으로 그 항목들의 메시지를 지운다. 게시(`postMessage`)가 끝나기 전에 항목이 먼저 종료되면, 뒤늦게 도착한 ts는 등록하지 않고 즉시 삭제한다. 프로세스 재시작으로 메시지 ts를 잃은 항목은 다음 `queue` 명령 출력에서 재게시되지 않고 이력으로만 남는다. |

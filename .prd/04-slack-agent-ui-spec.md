...
| A39 | 큐 항목 인라인 표시 (유저 2026-09-17 "thread안에 유저가 메세지 쳤을때마다 출력해줘") | 메시지가 큐에 들어가면(스티어링 성공 여부와 무관) 같은 스레드에 항목 메시지 1건을 게시한다: `n. 원문(≤80자) · 상태` + 버튼 `Send now`·`Cancel`(기존 action id·에포크 페이로드). 결합 패널에는 Queue 섹션을 렌더하지 않는다(헤더 breakdown의 `queued N`도 제거). 항목 메시지는 봇 게시이므로 패널 재앵커 알림을 발생시킨다. |
| A40 | 큐 확인 명령 (유저 2026-09-17 "따로 명령어 추가해주고 메세지 큐확인할수 있게") | `queue` / `큐` 명령은 그 스레드 세션의 미처리 항목(queued/steered/paused/uncertain/failed)만 나열한다(각각 A39 컨트롤 포함). 처리된 항목(resolved/cancelled)은 목록에 없다. 빈 큐면 `대기 중인 메시지가 없습니다` 1줄. 실행 중 턴을 끊지 않는 컨트롤 명령이다. |
| A41 | 처리된 항목 메시지 삭제 (유저 2026-09-17 "처리된 메세지는 삭제해줘") | 항목이 resolved(consumed 포함)/cancelled/failed가 되면 A39·A40이 게시한 그 항목의 메시지를 `chat.delete`로 지운다(베스트 에포트, 실패는 로그). 프로세스 재시작으로 메시지 ts를 잃은 항목은 다음 `queue` 명령 출력에서 재게시되지 않고 이력으로만 남는다. |

---
name: ui-ux
description: "UI/UX design intelligence for web and mobile. Includes 50+ styles, 161 color palettes, 57 font pairings, 161 product types, 99 UX guidelines, and 25 chart types across 10 stacks (React, Next.js, Vue, Svelte, SwiftUI, React Native, Flutter, Tailwind, shadcn/ui, and HTML/CSS). Actions: plan, build, create, design, implement, review, fix, improve, optimize, enhance, refactor, and check UI/UX code. Projects: website, landing page, dashboard, admin panel, e-commerce, SaaS, portfolio, blog, and mobile app. Elements: button, modal, navbar, sidebar, card, table, form, and chart. Styles: glassmorphism, claymorphism, minimalism, brutalism, neumorphism, bento grid, dark mode, responsive, skeuomorphism, and flat design. Topics: color systems, accessibility, animation, layout, typography, font pairing, spacing, interaction states, shadow, and gradient. Named design references: apply a saved real-world site reference by name (e.g. 'use the claude reference') — see 'Named Design References'; defaults to the 'openai' reference when none is named. Integrations: shadcn/ui MCP for component search and examples."
...
> Vendored from the MIT-licensed **UI/UX Pro Max** skill by NextLevelBuilder
> (`github.com/nextlevelbuilder/ui-ux-pro-max-skill`, v2.6.2). See `NOTICE.md`.
> Local additions: **Named Design References** (real-site `design.md` references,
...
All scripts and data ship inside this skill directory. Resolve it via the plugin
root and run every command from there:

```bash
UIUX="$CLAUDE_PLUGIN_ROOT/skills/ui-ux"   # bundled plugin path
# Fallback when CLAUDE_PLUGIN_ROOT is unset (e.g. running from a repo checkout):
[ -d "$UIUX" ] || UIUX="$(git rev-parse --show-toplevel 2>/dev/null)/plugin/local/skills/ui-ux"
```

In the command examples below, `python3 scripts/search.py …` means
`python3 "$UIUX/scripts/search.py" …` — the scripts resolve their CSV data
relative to their own location, so they work from any working directory.
...
A **named design reference** is a real-world site's visual language, captured as a
`design.md` file under `references/<name>/design.md` inside this skill. References
are authored by the companion **`ui-ux-reference`** skill (point it at any URL and
it extracts a `design.md`). Once saved, you reference them here **by name**.

### Default reference

- **`openai`** — OpenAI brand: clinical, high-contrast, engineered. Pure white
  canvas `#FFFFFF` + pure black ink `#000000` with no chromatic accent (emphasis by
  black fills / inversion and opacity tiers), a single grotesque sans ("OpenAI Sans"
  → Inter/Söhne stand-in), sharp `0px` corners, ruthless whitespace. **This is the
  default** — apply it whenever the user does not name another reference. See
  `references/openai/design.md`.
...
- **`claude`** — Anthropic / Claude brand: warm, editorial, calm. Ivory paper
  ...
  quiet motion. See `references/claude/design.md`.
...
When the user names a reference — e.g. *"build it with the il-capo-production
reference"*, *"use the `<name>` reference / design"*, or *"design like `<name>`"*:

1. **Resolve & read** the file:
   ```bash
   cat "$UIUX/references/<name>/design.md"
   ```
   If `<name>` is unknown, list `references/` (or read `references/index.json`)
   and ask which reference to use — do not silently fall back.
2. **Treat the reference as the visual North Star.** Its palette, typography,
   layout system, motion language, and signature techniques **override** the
   generic recommendations the `--design-system` engine would produce when they
   conflict. The engine output then fills the gaps the reference is silent on.
3. **Still run the quality gates.** The reference dictates *aesthetic direction*,
   not permission to break the CRITICAL/HIGH rules (accessibility, touch targets,
   performance). Adapt the reference's choices to pass Quick Reference §1–§3 —
...
`"default": true` in `references/index.json` — currently `openai`): read its
...
You do not author `design.md` by hand here — invoke the **`ui-ux-reference`**
skill with a URL and a short name. It extracts the site's design language into
`references/<name>/design.md` and registers it in `references/index.json`, after
which it is referenceable by `<name>` from this skill.

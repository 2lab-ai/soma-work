...
> author the file under `plugin/local/skills/ui-ux/references/<name>/` and open a PR
> (the same way the default `openai` reference ships). When running
> against a local checkout, write directly into `plugin/local/...`.

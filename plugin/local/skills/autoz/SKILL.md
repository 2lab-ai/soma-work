...
│   ├── core:structurize       — lossless structuring of the trace result
...
│       │   ├── local:oracle-reviewer + subagent (opus) — 3-person majority review voters
│       │   └── local:using-epic-tasks — Case A/B/C issue/epic routing
│       │       └── core:using-ha-thinking — layer discipline for artifact bodies
...
│       │   ├── core:simplify        — over-engineering pass on the diff
...
3. **Structurize.** Run `core:structurize` on the trace result — the analysis must be a lossless hierarchy, not a narrative.
...
5. **Analysis.** Analysis Step 1–4: derive tier surface → `stv:trace` → `core:structurize` → problem-analysis HTML artifact to the thread.
...
| `stv:trace` / `core:structurize` / `local:html` | Analysis Step engine — trace, structure, render. `local:html` applies the `ui-ux` skill (`openai` reference) as its design engine. |
...
| `codex-fallback` (Opus agent) | Automatic fallback2 of the trinity chain (`plugin/local/agents/codex-fallback.md`), spawned when both the panel and codex are unavailable. Verdict logged as `trinity-fallback2 (opus)`. |

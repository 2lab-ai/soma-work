...
- **빌드 산출물 디렉토리 직접 수정 금지**: `dist/`, `build/`, `out/` 등 빌드가 생성하는 디렉토리에 파일을 직접 생성·수정·삭제하지 마라. 소스 파일을 고치고 빌드 프로세스를 통해 반영한다. `dist/`에 직접 쓰는 것은 배포물에 낙서하는 것과 같다.
- **`node_modules/` 직접 수정 금지**: 패키지 매니저(`npm install`)를 통해서만 관리한다. 직접 파일을 만들거나 고치지 마라.

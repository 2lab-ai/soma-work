...
- **`dist/` 디렉토리 직접 수정 금지**: `dist/`는 빌드 산출물이다. 여기에 파일을 생성, 수정, 삭제하지 마라. 모든 변경은 반드시 `src/`에서 하고 빌드 프로세스를 통해 `dist/`에 반영한다. `dist/`에 직접 쓰는 것은 배포물에 낙서하는 것과 같다.
- **빌드 산출물에 직접 작업 금지**: `dist/`, `build/`, `out/`, `node_modules/` 등 생성된 디렉토리에 직접 파일을 만들거나 고치지 마라. 소스(`src/`)를 고치고, 빌드하고, PR을 만들어라.
- **소스 변경 워크플로우 필수**: 코드 변경 시 반드시 → ① git clone (유니크 폴더) → ② 브랜치 생성 → ③ `src/` 수정 → ④ 커밋 → ⑤ PR 생성. 이 절차를 건너뛰지 마라.

...
describe('TokenManager (AuthKey v2, keyId-keyed)', () => {
  ...
  // ── #871 — Cooldown propagation removed ─────────────────
  //
  // The shared-bucket inference (#804/#869) was deleted. A 429 marks ONLY the
  // active slot. Sibling slots' state is untouched. This test pins the new
  // contract; it would FAIL while the propagation helper still existed.
  describe('rotateOnRateLimit > no cross-slot cooldown propagation (#871)', () => {
    ...
    it('first 429 marks only the active slot; siblings remain pristine', async () => {
      const { mod, storeMod } = await importSut();
      const store = new storeMod.CctStore(path.join(tmp, 'cct-store.json'));
      const tm = new mod.TokenManager(store);
      await tm.init();
      const [a, b, c] = await addSixOauthSlots(tm);
      await tm.rotateOnRateLimit('first', { source: 'error_string', cooldownMinutes: 60 });
      const snap = await store.load();
    ...
    it('second 429 within window does NOT propagate to other siblings (the bug operators reported)', async () => {
      // Production caller passes a parsed wall-clock 429 from `error_string`.
      // Prior code (#804/#869) treated this as a shared-bucket signal and
      // pre-marked every healthy sibling with `inferred_shared` cooldownUntil.
      // The new contract: only the active slot is touched. Each sibling has
      // to receive its own 429 to be marked.
      const { mod, storeMod } = await importSut();
      const store = new storeMod.CctStore(path.join(tmp, 'cct-store.json'));
      const tm = new mod.TokenManager(store);
      await tm.init();
      const [a, b, c, d, e, f] = await addSixOauthSlots(tm);
      // 1st call: marks A, rotates active → B.
      await tm.rotateOnRateLimit('first', { source: 'error_string', cooldownMinutes: 60 });
      // 2nd call: marks B (currentId now). Same wall-clock window.
      await tm.rotateOnRateLimit('second', { source: 'error_string', cooldownMinutes: 60 });
      const snap = await store.load();
      // A and B got direct marks — that's the active-slot path; unchanged.
      expect(snap.state[a.keyId].rateLimitSource).toBe('error_string');
      expect(snap.state[a.keyId].cooldownUntil).toBeDefined();
      expect(snap.state[b.keyId].rateLimitSource).toBe('error_string');
      expect(snap.state[b.keyId].cooldownUntil).toBeDefined();
      // CRITICAL: C..F MUST remain pristine. No `inferred_shared`, no cooldown.

...
describe('SessionRegistry persistence', () => {
  ...
  it('persists linkHistory and sequence for session resources', () => {
    ...
    writer.setSessionLink('C123', '171.003', {
      url: 'https://jira.example/PROJ-100',
      type: 'issue',
      provider: 'jira',
      label: 'PROJ-100',
    });
    writer.setSessionLink('C123', '171.003', {
      url: 'https://jira.example/PROJ-101',
      type: 'issue',
      provider: 'jira',
      label: 'PROJ-101',
    });
    ...
    expect(snapshot.active.issue?.url).toBe('https://jira.example/PROJ-101');
      ...
      operations: [
        {
          ...
          resourceType: 'issue',
          link: {
            url: 'https://jira.example/PROJ-200',
            type: 'issue',
            provider: 'jira',
          },
        },
  ...
  it('keeps active link cleared when set_active is called without url', () => {
      ...
      operations: [
        {
          ...
          resourceType: 'issue',
          link: {
            url: 'https://jira.example/PROJ-300',
            type: 'issue',
            provider: 'jira',
          },
        },
    ...
    expect(addResult.snapshot.active.issue?.url).toBe('https://jira.example/PROJ-300');
...
describe('SessionRegistry deserialize — model coerce', () => {
  ...
  it('coerces unknown model ids to DEFAULT_MODEL', () => {
    ...
    expect(restored?.model).toBe('gpt-5.6-sol'); // DEFAULT_MODEL
  });

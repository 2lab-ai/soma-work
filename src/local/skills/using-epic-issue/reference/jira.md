...
- **본 스킬에서 "서브이슈" = Story/Task (L0).** Sub-task(L-1)는 쓰지 않음.
...
근거: Sub-task는 parent Story와 같은 PR 단위로 묶이기 쉬워 "1 체크박스 = 1 PR = 1 서브이슈" (Invariant 2)가 깨진다. Story로 분해해야 각자 독립 PR·독립 워크트리·독립 에이전트 세션이 가능.

- legacy `Epic Link` 커스텀 필드는 **deprecated** → `parent` 필드로 통합 (custom field ID는 사이트마다 다름 — 하드코딩 금지).
- Premium은 Advanced Roadmaps로 상위 계층 확장 가능.
...
## 4. Sub-task vs Linked issue

| 구분 | 용도 |
|---|---|
| Sub-task | 한 부모의 하위 분해 작업. 동일 프로젝트 한정. work log roll-up. |
| Linked issue | 비계층 관계 (`blocks`/`relates`/`duplicates`). cross-project 가능. |

원칙: "완료 전 반드시 끝나야 할 분해 작업" = sub-task. "의존/참조" = link.

```bash
curl -X POST "https://$SITE/rest/api/3/issueLink" \
  -d '{"type":{"name":"Blocks"},"inwardIssue":{"key":"PRJ-2"},"outwardIssue":{"key":"PRJ-1"}}'
```

ref: <https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-issue-links/> · <https://support.atlassian.com/jira-software-cloud/docs/create-an-issue-and-a-sub-task/>

## 5. Epic close — 수동 전이 (자동화 금지)
...
- **`Labels`** — `epic`, `blocked`, `ready` 등. **상태 라벨은 서브이슈에만** — 에픽에 붙이면 본문 불변성(Invariant 1)을 우회하는 암묵적 상태가 생긴다.
...
- DC/Server: `/rest/api/2/` 유지, `"Epic Link" = PRJ-1` JQL 가능.

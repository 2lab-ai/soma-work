---
name: design
description: High-craft visual design via HTML — hi-fi prototypes, interactive demos, slide decks, animations, design-variation exploration, a design-direction advisor and a 5-dimension expert critique. Embodies the right specialist per task (UX / motion / slide / prototype designer) and aggressively avoids AI-slop web-design tropes (purple gradients, emoji icons, Inter-as-display, CSS silhouettes for real product shots). Used standalone for design work, and consumed by local:html for its anti-AI-slop discipline + 20-philosophy style vocabulary (see references/design-styles.md). Triggers on "디자인 해줘", "프로토타입", "목업", "mockup", "prototype", "hi-fi design", "디자인 방향", "스타일 추천", "design direction", "design styles", "예쁘게 만들어줘", "make it look good", "review this design", "做原型", "设计Demo", "HTML演示", "设计风格", "设计方向", "配色方案", "评审". Core capabilities: Junior-Designer workflow (assumptions + reasoning + placeholders, show early, then iterate), Fact-Verification-First (WebSearch before claiming a product/version exists), Core Asset Protocol (logo / product shots / UI screenshots before pixels), anti-AI-slop checklist, Tweaks live-variation switching, Starter Components (device bezels / deck stage / variation canvas), App-prototype protocol (real images + Playwright click tests), and the optional HTML→MP4/GIF animation + narration pipeline. Vendored from alchaincyf/huashu-design (MIT) — see NOTICE.md.
allowed-tools: Read, Write, Edit, Bash, Glob, Grep, WebSearch, WebFetch
version: 1.0.0
license: MIT
---

# Design · 花叔Design (vendored as soma-work `design` skill)

> **Vendoring note (soma-work).** This skill is vendored from
> [`alchaincyf/huashu-design`](https://github.com/alchaincyf/huashu-design)
> under the MIT License (see [`NOTICE.md`](./NOTICE.md) / [`LICENSE`](./LICENSE)).
> The 6 background-music tracks (`assets/bgm-*.mp3`, ~27 MB) are **NOT** bundled —
> they are video-pipeline-only binaries unused by soma-work's Slack `HTML → PNG`
> flow and would bloat the repo. To run the MP4/BGM export pipeline, download them
> from the upstream release (`scripts/add-music.sh` / `references/video-export.md`)
> into `assets/`. The 37 lightweight SFX clips (`assets/sfx/`) and all text
> references ARE bundled. The Korean/Chinese/English bodies below are the upstream
> agent prompts, kept verbatim — the agent is bilingual and the structural intent
> translates regardless of the prose language.

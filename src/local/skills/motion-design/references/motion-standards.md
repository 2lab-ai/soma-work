...
watching most. `ease-out` at 200ms *feels* faster than `ease-in` at 200ms.
...
Instant tooltips after the first one (skip delay + animation) make a toolbar feel
faster.
...
- **Don't drive child transforms via a CSS variable on the parent** — it recalcs
  ...
  ```
- **Library shorthand transforms may not be hardware-accelerated.** (Example:
  Framer Motion's `x`/`y`/`scale` props run on the main thread via rAF and drop
  frames under load.) Prefer a full transform string for predetermined motion:
  ```jsx
...
- **CSS animations beat JS under load** — they run off the main thread;
  ...
  for predetermined motion, JS/springs for dynamic/interruptible.

...
**VirtualSport 정산 파이프라인**에서 `NotifySettlements` 메시지가 SnapshotService에서 SettlementService/projalphaService로 **WebSocket을 통해 전달되지 않는** 장애가 의심되었다.
...
| 시각 (UTC) | 이벤트 |
|-----------|--------|
| 03/31 06:49 | 이전 세션에서 PROJ-3231 작업 시작 |
| 03/31 07:14 | Jira PROJ-3231 이슈 생성 (7개 장애포인트 분석) |
...
| **Jira PROJ-3231** | QA | 배포 후 실제 정산 동작 QA 검증 필요 |

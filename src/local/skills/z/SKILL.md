...
4. Get the plan reviewed via the `local:trinity` chain (trinity 3-engine consensus → `llm_chat codex` → `codex-fallback` opus). Pass = unanimous APPROVE with MUST-FIX none; on the fallback tiers, pass = score ≥ 95. Below the bar, update the plan from the feedback and resubmit for review — repeat until it passes.

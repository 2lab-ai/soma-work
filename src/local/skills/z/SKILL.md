...
- `ci-fail-after-*` (e.g. `ci-fail-after-3-cycles` — the merge or §5.2 subagent's own CI-fix budget is exhausted) → escalate via `UIAskUserQuestion`. Do **not** dispatch §5.2 — CI failure isn't a conflict; the user decides whether to extend the fix budget, hand-debug, or abort. Same routing whether the `detail` originates from §5.1 or §5.2.
...
Triggered **only by the conflict-class branch of §5.1.a's blocker taxonomy** — i.e. `detail` matches `unresolvable-conflict-on-shared-file` or `rebase-conflict-*`. Other blocker classes (`reviewDecision-regressed-*`, `branch-protection-blocked`, `signed-commit-required-no-key`, etc.) do **not** dispatch §5.2; they take their own routing path defined in §5.1.a's taxonomy. The orchestrator dispatches a **separate conflict-resolution subagent** (background) — see `reference/samples/05-rebase-merge-conflict.md` — that handles rebase + force-with-lease + reviewDecision recheck + merge. The orchestrator does not resolve conflicts itself.

**Report contract** — the §5.2 subagent's final report MUST use the same shape as §5.1's merge subagent: a discriminated `{ status: 'MERGED', mergeCommitSha } | { status: 'blocker', detail }`. On `MERGED` the orchestrator routes through §5.1.a (next-group base-refresh OR final closeout) — **do not re-dispatch the §5.1 merge driver**, the conflict subagent already merged the PR. On `blocker`, **§5.1.a's taxonomy is re-applied to §5.2's report by `detail`**: a non-conflict `detail` (e.g. `reviewDecision-regressed-after-force-push` from §5.2's own force-push) routes to that detail's branch (phase 4 / `UIAskUserQuestion`) — it does not loop back into §5.2. Only a repeated conflict-class `detail` from §5.2 hits the bounded retry → escalation path (max 2 §5.2 dispatches per PR).

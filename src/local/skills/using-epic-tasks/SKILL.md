...
- `using-ssot` — SSOT-TASK-TREE 정의. **`z` phase0이 이미 만들어 둔 트리가 이 스킬의 1차 입력**. 새로 만들지 않는다.
- `decision-gate` — 규모(switching cost) 판정
- `using-ha-thinking` — 각 산출물 본문의 언어 규율

## SSOT-TASK-TREE → Case 매핑

phase0이 넘긴 SSOT-TASK-TREE를 그대로 사용한다. 케이스 매핑은 트리 모양 + decision-gate tier 결합. **트리 모양 신호는 `local:decision-gate` §"SSOT-TASK-TREE shape as a tier signal" 표를 단일 진실원으로 따른다 — 여기서 재정의하지 않는다.**

| decision-gate tier (트리 신호와 line-count 결합) | 케이스 | 매핑 |
|---|---|---|
| `tiny` ~ `large` | **Case A** | 트리 전체 = 이슈 1 + PR 1. `ssot-task`는 PR body `## SSOT-TASK-TREE coverage`의 행으로. `ssot-subtask`는 commit message 단위에만 (PR body / 이슈 body 금지 — `using-ssot` Invariant 4). |
| `xlarge` | **Case B** | 트리 root = 에픽. 각 `ssot-task` = 서브이슈 1개. 한 서브이슈는 1 PR로. `ssot-subtask`는 commit message에만. |
| `xxlarge` | **Case C** | 트리 그대로를 유저에게 분해 제안으로 제출. 승인되면 root의 상위 그룹을 N개 에픽으로 쪼개고 각각 독립 세션에서 Case B로. |

ssot-task ID는 산출물 안에서 carry — 서브이슈 본문 첫 줄 `<!-- ssot-task: T3 -->`, PR body `## SSOT-TASK-TREE coverage`에 같은 ID로 라벨링. 추적이 시각화됨.
...
2. 3명 리뷰(self + `oracle-reviewer` + `oracle-gemini-reviewer`)로 가능한 분해 구조 초안. **SSOT-TASK-TREE를 분해 제안의 출발점으로 사용** — root 아래 `ssot-task` 그룹화가 곧 에픽 후보. 새로 분해를 발명하지 말고 트리 모양을 우선.
3. 유저에게 보고 — "이 요청은 에픽 N개 분량입니다. SSOT-TASK-TREE를 이렇게 쪼개는 것을 제안합니다: [에픽 목록 + 각 에픽이 커버할 ssot-task ID 집합]. 승인하시겠습니까?"
4. 유저 승인 후 — 각 에픽을 독립 세션에서 Case B로 처리. 각 에픽 세션은 자기 `ssot-task` 부분집합만 SSOT-TASK-TREE의 view로 받는다.

...
export class SlackBlockKitChannel implements NotificationChannel {
  ...
  async send(event: TurnCompletionEvent): Promise<void> {
    ...
    try {
      ...
      // Trace: docs/current/plans/turn-summary-lifecycle/trace.md, S6
  ...
  private buildBlocksForTheme(theme: SessionTheme, event: TurnCompletionEvent, emoji: string, label: string): any[] {
    switch (theme) {
      case 'default':
        return this.buildDefaultBlocks(event, emoji, label);
      case 'compact':
        return this.buildCompactBlocks(event, emoji, label);
      case 'minimal':
        return this.buildMinimalBlocks(event, emoji, label);
      default:
        return this.buildDefaultBlocks(event, emoji, label);
    }
  }
  ...
  private buildDefaultBlocks(event: TurnCompletionEvent, emoji: string, label: string): any[] {
    ...
    // rich-context (model/usage/tools) lines. PROJ-4318 / soma-work#933.
    ...
    if (has5h) {
      ...
      usageParts.push(
        `5h ${this.renderBar(event.fiveHourUsage!, 6)} ${Math.round(event.fiveHourUsage!)}%${delta5h ? ` ${delta5h}` : ''}`,
      );
    }
    if (has7d) {
      const delta7d = this.formatSignedDelta(event.sevenDayDelta, 0);
      usageParts.push(
        `7d ${this.renderBar(event.sevenDayUsage!, 8)} ${Math.round(event.sevenDayUsage!)}%${delta7d ? ` ${delta7d}` : ''}`,
      );
    }
  ...
  private buildCompactBlocks(event: TurnCompletionEvent, emoji: string, label: string): any[] {
    ...
    // benefits from seeing the multi-line reason. PROJ-4318 / soma-work#933.
    const bodyBlock = this.buildExceptionBodyBlock(event);
    if (bodyBlock) blocks.push(bodyBlock);
  ...
  private formatToolStatsRich(stats: Record<string, { count: number; totalDurationMs: number }>): string | undefined {
    ...
    const parts = entries.slice(0, 5).map(([name, s]) => {

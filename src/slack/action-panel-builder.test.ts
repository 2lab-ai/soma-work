...
describe('ActionPanelBuilder', () => {
  ...
  it('renders dialog text without thread permalink preview text', () => {
    const payload = ActionPanelBuilder.build({
      ...
      workflow: 'default',
      panelTitle: 'PROJ-2411',
      styleVariant: 3,
    });

    const allText = JSON.stringify(payload.blocks);
    expect(allText).toContain('PROJ-2411');
  ...
  it('renders real clickable links in a separate context block', () => {
    const payload = ActionPanelBuilder.build({
      ...
      links: {
        issue: {
          type: 'issue',
          provider: 'jira',
          url: 'https://jira.example.com/browse/PROJ-2411',
          label: 'PROJ-2411',
        },
        pr: {
          type: 'pr',
    ...
    expect(contextBlock.elements[0].text).toContain('<https://jira.example.com/browse/PROJ-2411|PROJ-2411>');

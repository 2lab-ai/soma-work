...
이미지는 파일 이름과 메타데이터(mimetype, size)만 참조하고, 유저에게 이미지 내용을 설명해달라고 요청하세요.
  ...
  async execute(params: StreamExecuteParams): Promise<ExecuteResult> {
    ...
    try {
      ...
      if (streamResult.sdkResultError && streamResult.sdkResultError.errors.length > 0) {
        const { subtype, errors, numTurns } = streamResult.sdkResultError;
        const errorLines = [
          `⚠️ *[SDK Result Error]* ${subtype}`,
      ...
      // Reset error retry count and error context on success
  ...
  private extractBlockedPath(error: any): string | undefined {
    ...
    const match = combined.match(/(?:file access blocked|access blocked):\s*(.+?)(?:\n|$)/i);
    return match?.[1]?.trim();
  }
  ...
  private formatErrorForUser(error: any, sessionCleared: boolean, statusInfo?: import('../../claude-status-fetcher').ClaudeStatusInfo | null, retryAttempt?: number): string {
    ...
    if (error.stderrContent) {
      ...
      const sanitized = raw
        ...
        .replace(/(?:oauth|token|key|secret|authorization|bearer|password|credential)[=:\s]+\S+/gi, '[REDACTED]')
        .replace(/\bsk-ant-[a-zA-Z0-9_-]+/g, '[REDACTED]')      // Anthropic API keys
        .replace(/\bxox[bpras]-[a-zA-Z0-9-]+/g, '[REDACTED]')   // Slack tokens
        .replace(/\bgh[pus]_[a-zA-Z0-9]+/g, '[REDACTED]');       // GitHub tokens

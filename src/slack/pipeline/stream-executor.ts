  ...
  async execute(params: StreamExecuteParams): Promise<ExecuteResult> {
    ...
    try {
      ...
      if (streamResult.sdkResultError) {
        ...
        if (errors.length > 0) {
          // Cap at 5 items, truncate each to 200 chars
      ...
      // Reset error retry count on success (skip if SDK reported an error)
      if (!hasSdkError) {
  ...
  private formatErrorForUser(error: any, sessionCleared: boolean, statusInfo?: import('../../claude-status-fetcher').ClaudeStatusInfo | null, retryAttempt?: number): string {
    ...
    if (error.stderrContent) {
      ...
      const sanitized = raw
        ...
        .replace(/(?:authorization|bearer)[=:\s]+\S+(?:\s+\S+)?/gi, '[REDACTED]')  // auth headers (captures "Bearer <token>")

...
// ── Blocks too long error recovery ──

describe('Blocks too long error recovery', () => {
  function createExecutorDeps() {
    return {
      claudeHandler: {
        setActivityState: vi.fn(),
  ...
  // ── isBlocksTooLongError unit tests ──

  it('detects msg_blocks_too_long in error.data.error', () => {
    const executor = new StreamExecutor({} as any);
    const error = new Error('An API error occurred: msg_blocks_too_long');
    (error as any).data = { error: 'msg_blocks_too_long' };
    expect((executor as any).isBlocksTooLongError(error)).toBe(true);
  });

  it('detects msg_blocks_too_long in error message only', () => {
    const executor = new StreamExecutor({} as any);
    const error = new Error('An API error occurred: msg_blocks_too_long');
    expect((executor as any).isBlocksTooLongError(error)).toBe(true);
  });
  ...
  it('does NOT match unrelated errors', () => {
    const executor = new StreamExecutor({} as any);
    const error = new Error('Some completely different error');
    expect((executor as any).isBlocksTooLongError(error)).toBe(false);
  });

  it('does NOT match msg_too_long (different error)', () => {
    const executor = new StreamExecutor({} as any);
    const error = new Error('msg_too_long');
    expect((executor as any).isBlocksTooLongError(error)).toBe(false);
  });

  // ── handleError integration tests ──

  it('preserves session and stores error context on blocks too long', async () => {
    const deps = createExecutorDeps();
    const executor = new StreamExecutor(deps);
    const say = vi.fn().mockResolvedValue(undefined);
    const session = {} as any;
    ...
    // Session NOT cleared
    expect(deps.claudeHandler.clearSessionId).not.toHaveBeenCalled();
    // Error context stored on session — tells model to shorten response
    expect(session.lastErrorContext).toContain('msg_blocks_too_long');
    expect(session.lastErrorContext).toContain('짧고 간결하게');
    // Retry scheduled with short delay (3s)
    expect(retryAfterMs).toBe(3_000);
    // Error message shown to user
    expect(say).toHaveBeenCalledTimes(1);
    const payload = say.mock.calls[0][0];
    expect(payload.text).toContain('블록 크기 제한');
    expect(payload.text).toContain('자동 재시도');
  });

  it('exhausts retry budget after MAX_ERROR_RETRIES attempts', async () => {
    const deps = createExecutorDeps();
    const executor = new StreamExecutor(deps);
    const say = vi.fn().mockResolvedValue(undefined);
    const session = { blocksTooLongRetryCount: 3 } as any;
    const error = new Error('An API error occurred: msg_blocks_too_long');
    (error as any).data = { error: 'msg_blocks_too_long' };
    ...
    expect(retryAfterMs).toBeUndefined();
    expect(session.blocksTooLongRetryCount).toBe(0);
    expect(session.lastErrorContext).toBeUndefined();
  });

  it('increments blocksTooLongRetryCount on each retry', async () => {
    const deps = createExecutorDeps();
    const executor = new StreamExecutor(deps);
    const say = vi.fn().mockResolvedValue(undefined);
    const session = { blocksTooLongRetryCount: 1 } as any;
    const error = new Error('An API error occurred: msg_blocks_too_long');
    (error as any).data = { error: 'msg_blocks_too_long' };

    await (executor as any).handleError(error, session, 'K', 'C', 't', [], say);

    expect(session.blocksTooLongRetryCount).toBe(2);
  });
});

...
import { VERBOSITY_NAMES, type LogVerbosity, getVerbosityFlags } from '../output-flags';
...
export class VerbosityHandler implements CommandHandler {
  ...
  async execute(ctx: CommandContext): Promise<CommandResult> {
    ...
    if (parsed.action === 'set' && parsed.level) {
      ...
      const levelList = VERBOSITY_NAMES
        .map(name => name === current ? `• *${name}* _(current)_` : `• ${name}`)
        .join('\n');

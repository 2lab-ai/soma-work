...
import { EchartsInitError, FontLoadError, ResvgNativeError, SlackUploadError } from '../../metrics/usage-render/errors';
import type { UsageCardStats } from '../../metrics/usage-render/types';
import type { CommandContext, CommandDependencies } from './types';
import { isCardSubcommand, type UsageCardOverrides, UsageHandler } from './usage-handler';
...
function makeOverrides(
  ...
  const overrides: UsageCardOverrides = {
    ...
    slackApi: { filesUploadV2, postMessage, postEphemeral },
    clock: () => new Date('2026-04-17T14:00:00+09:00'),
  };
  return { overrides, aggregator, renderer, filesUploadV2, postMessage, postEphemeral };
}
...
describe('UsageHandler.handleCard — privacy gate', () => {
  it('rejects `/usage card @someone_else`', async () => {
    ...
    expect((deps.slackApi as any).postSystemMessage).toHaveBeenCalledWith(
      'C_TEST',
      expect.stringMatching(/다른 사용자의 사용량 카드는 조회할 수 없습니다/),
      expect.anything(),
    );
  });
});
...
describe('UsageHandler.handleCard — error fallback', () => {
  ...
  it('SlackUploadError wraps uploadV2 failure → ephemeral fallback', async () => {

...
describe('ReportFormatter', () => {
  ...
  it('enrichedWeekly_baselineZero_showsFirstRecord', () => {
    const result = formatter.formatEnrichedWeekly({
      ...
      trend: {
        sessionsCreatedDelta: 100,
        turnsUsedDelta: 100,
        prsCreatedDelta: 100,
        commitsCreatedDelta: 100,
        codeLinesAddedDelta: 100,
        prsMergedDelta: 100,
        productivityScoreDelta: 100,
        baselineZero: true,
      },
      dailyBreakdown: Array.from({ length: 7 }, (_, i) => ({
        date: `2026-03-${23 + i}`,
  ...
  it('enrichedWeekly_blockCountNeverExceeds50', () => {
    ...
    const result = formatter.formatEnrichedWeekly({
      ...
      derived: {
        productivityScore: 999,
        prMergeRate: 80,
        avgCodePerPr: 500,
        avgCodePerCommit: 100,
        avgTurnsPerSession: 8,
        sessionCompletionRate: 90,
        netLines: 4500,
        churnRatio: 15,
        avgChangedLinesPerPr: 600,
        commitPerActiveDay: 12,
        prPerActiveDay: 3,
      },
      trend: {
        sessionsCreatedDelta: 20,
        turnsUsedDelta: 15,
        prsCreatedDelta: 30,
        commitsCreatedDelta: 25,
        codeLinesAddedDelta: 40,
        prsMergedDelta: 50,
        productivityScoreDelta: 35,
        baselineZero: false,
      },
      dailyBreakdown: Array.from({ length: 7 }, (_, i) => ({
        date: `2026-03-${23 + i}`,
        dayLabel: ['월', '화', '수', '목', '금', '토', '일'][i],
        totalEvents: 100 + i * 20,
        metrics: makeMetrics(),
      })),

    ...
    routes['/api/triage'] = json(
      triageReport([
        triageIssue({
          ...
          text: 'proj-dev2-api',
          detail: 'HTTP 503',
          id: 'CHECK|proj-dev2-api',
        }),
      ]),
    );
    routes['/api/snapshot'] = json(snapshot());

    const evidence = await collect({ incident_id: 'CHECK|proj-dev2-api' });

    expect(evidence.snapshot?.check).toEqual({
      ref: 'eagle:/api/snapshot#http_checks[name=proj-dev2-api]',
      name: 'proj-dev2-api',
      ok: false,
      status: 503,
      latency_ms: 91,
  ...
  it('a stale row stays stale under a freshly collected batch clock', async () => {
    ...
    routes['/api/triage'] = json(
      triageReport([
        ...
        triageIssue({ cat: 'CHECK', key: undefined, text: 'proj-dev2-api', id: 'check-id' }),
      ]),
    );
    const freshBatch = {

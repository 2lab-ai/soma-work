...
describe('buildCompactionContext', () => {
  ...
  it('includes session title', () => {
    const result = buildCompactionContext({ ...baseSnapshot, title: '[PROJ-123] Fix login bug' });
    expect(result).toContain('Session: [PROJ-123] Fix login bug');
  });
  ...
  it('includes linked resources with label, title, and status', () => {
    ...
    expect(result).toContain('- [issue] PROJ-123 "Fix login redirect" https://jira.example.com/PROJ-123 (in-progress)');

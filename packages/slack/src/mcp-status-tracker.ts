...
export interface McpCallTrackerReader {
...
function computeAdaptivePrediction(
  elapsed: number,
...
export class McpStatusDisplay {
  ...
  constructor(
    ...
    private mcpCallTracker: McpCallTrackerReader,
  ) {}
  ...
  private renderCallLine(call: ActiveCallEntry): string {
    ...
    if (call.status === 'completed' || call.status === 'failed') {
      ...
      if (call.duration !== undefined) {
        line += ` (${formatDuration(call.duration)})`;
      }
      return line;
    }
    ...
    const elapsed = this.mcpCallTracker.getElapsedTime(call.callId);
    const elapsedMs = elapsed ?? Date.now() - call.startTime;
    let line = `⏳ ${call.config.displayLabel}${params} — ${formatDuration(elapsedMs)}`;

    if (call.predicted !== null && call.predicted > 0) {
      const adaptive = computeAdaptivePrediction(elapsedMs, call.predicted);
  ...
  private static formatAdaptiveIndicator(original: number, adjusted: number): string {
    return ` _🐢 ${formatDuration(original)} → ${formatDuration(adjusted)}_`;
  }
}

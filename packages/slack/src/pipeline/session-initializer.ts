...
interface RouteCheckResult {
  correct: boolean;
  suggestedChannels: ChannelInfoLike[];
  reason: string;
}
...
interface DispatchServiceLike {
  getModel(): string;
  isReady(): boolean;
  dispatch(text: string, signal: AbortSignal): Promise<any>;
}

export interface SessionInitializerProviders {
  expectedHandoffKind?: (workflow: WorkflowType) => string;
  isZHandoffWorkflow?: (workflow: WorkflowType | undefined) => workflow is ZHandoffWorkflow;
  parseHandoff?: (prompt: string) => HandoffParseResult;
  createHandoffAbortError?: (reason: string, detail: string, workflow?: WorkflowType) => Error;
  createDispatchAbortError?: (
  ...
  getAdminUsers?: () => Set<string>;
  checkRepoChannelMatch?: (prUrl: string, channel: string) => RouteCheckResult;
  getAllChannels?: () => ChannelInfoLike[];
  getChannel?: (channel: string) => ChannelInfoLike | null | undefined;
  registerChannel?: (client: any, channel: string) => Promise<unknown>;
  ...
  getDispatchService?: () => DispatchServiceLike;
  getUserSettings?: (userId: string) => any;
  createPendingUser?: (userId: string, userName: string) => void;
...
const missingProvider = (name: string): never => {
  throw new Error(`SessionInitializer provider is not configured: ${name}`);
};

const providers: Required<SessionInitializerProviders> = {
  expectedHandoffKind: () => missingProvider('expectedHandoffKind'),
  isZHandoffWorkflow: (workflow): workflow is ZHandoffWorkflow => false,
  parseHandoff: () => missingProvider('parseHandoff'),
  createHandoffAbortError: (reason, detail, workflow) => {
    const error = new Error(detail || reason) as Error & { reason?: string; forceWorkflow?: WorkflowType };
    error.reason = reason;
    error.forceWorkflow = workflow;
    return error;
  },
  ...
  getAdminUsers: () => new Set(),
  checkRepoChannelMatch: () => missingProvider('checkRepoChannelMatch'),
  getAllChannels: () => [],
  getChannel: () => undefined,
  registerChannel: async () => undefined,
  createConversation: () => missingProvider('createConversation'),
  getConversationUrl: () => missingProvider('getConversationUrl'),
  scheduleLinkDerivedTitleRefresh: () => undefined,
  getDispatchService: () => missingProvider('getDispatchService'),
  getUserSettings: () => undefined,
  createPendingUser: () => undefined,
...
export function setSessionInitializerProviders(next: SessionInitializerProviders): void {
  if (next.expectedHandoffKind) providers.expectedHandoffKind = next.expectedHandoffKind;
  if (next.isZHandoffWorkflow) providers.isZHandoffWorkflow = next.isZHandoffWorkflow;
  if (next.parseHandoff) providers.parseHandoff = next.parseHandoff;
  if (next.createHandoffAbortError) providers.createHandoffAbortError = next.createHandoffAbortError;
  if (next.createDispatchAbortError) providers.createDispatchAbortError = next.createDispatchAbortError;
  if (next.getAdminUsers) providers.getAdminUsers = next.getAdminUsers;
  if (next.checkRepoChannelMatch) providers.checkRepoChannelMatch = next.checkRepoChannelMatch;
  if (next.getAllChannels) providers.getAllChannels = next.getAllChannels;
  if (next.getChannel) providers.getChannel = next.getChannel;
...
export class SessionInitializer {
  ...
  async initialize(
    ...
    if (isNewSession) {
      ...
      try {
        const conversationId = providers.createConversation(channel, threadTs, user, userName);
        session.conversationId = conversationId;

        // Always show conversation URL — it's essential for session review
        const conversationUrl = providers.getConversationUrl(conversationId);
      ...
      const userSettings = providers.getUserSettings(user);
      if (!userSettings) {
        ...
        providers.createPendingUser(user, userName);
    ...
    if (event.skipDispatch && this.deps.claudeHandler.needsDispatch(channel, threadTs)) {
      ...
      if (forceWorkflow) {
        ...
        if (!ok) {
          throw providers.createDispatchAbortError(
            'transition-failed',
    ...
    if (shouldRoute && prUrl) {
      ...
      if (!providers.getChannel(channel)) {
        this.logger.info('🔀 Channel not in registry, registering on-the-fly', { channel });
        await providers.registerChannel(this.deps.slackApi.getClient(), channel);
      }

      const routeCheck = providers.checkRepoChannelMatch(prUrl, channel);
      ...
      if (!routeCheck.correct && routeCheck.suggestedChannels.length > 0) {
        ...
        const currentChannelInfo = providers.getChannel(channel);
  ...
  async runDispatch(
    ...
    if (providers.isZHandoffWorkflow(forceWorkflow) && this.deps.claudeHandler.needsDispatch(channel, threadTs)) {
      if (!handoffPrompt) {
        throw providers.createHandoffAbortError(
          'no-sentinel',
          'runDispatch received no handoffPrompt for forced z-* workflow',
          forceWorkflow,
        );
      }
      const parsed = providers.parseHandoff(handoffPrompt);
      if (!parsed.ok) {
        throw providers.createHandoffAbortError(parsed.reason, parsed.detail ?? '', forceWorkflow);
      }
      const expected = providers.expectedHandoffKind(forceWorkflow);
      ...
      if (!session) {
        throw providers.createHandoffAbortError('host-policy', 'session not found at handoff entry', forceWorkflow);
      }
  ...
  private async dispatchWorkflow(
    ...
    try {
      ...
      if (result.links && Object.keys(result.links).length > 0) {
        ...
        providers.scheduleLinkDerivedTitleRefresh(this.deps.claudeHandler, channel, threadTs, 'dispatch-entry');
      } else {
  ...
  private resolveDefaultRouteChannel(currentChannel: string): { id: string; name: string } | undefined {
    const configured = providers.getDefaultUpdateChannel()?.trim();
    if (!configured) {
      return undefined;
    }

    const channels = providers.getAllChannels();
  ...
  private async createBotInitiatedThread(
    ...
    const headerPayload = ThreadHeaderBuilder.build({
      ...
      links: session.links,
      theme: providers.getUserSessionTheme(user),
    });
  ...
  private async postMigratedContextSummary(
    ...
    if (session.conversationId) {
      const conversationUrl = providers.getConversationUrl(session.conversationId);
  ...
  private async postRouteAdvisory(
    ...
    const { text: advisoryText, blocks } = providers.buildChannelRouteBlocks(routeBlockParams);
    ...
    if (advisoryResult?.ts) {
      const updatedBlocks = providers.buildChannelRouteBlocks({
        ...routeBlockParams,
  ...
  private async notifyAdminsNewUser(userId: string, userName: string): Promise<void> {
    const adminUsers = providers.getAdminUsers();

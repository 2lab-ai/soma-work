...
type FileHandler = any;
type Continuation = any;
type ModelCommandPayloadMap = Record<string, any>;
type ModelCommandResult = any;
type SaveContextResultPayload = any;
type SessionResourceUpdateRequest = any;
type SessionUsage = any;
...
type OneMUnavailableKind = 'entitlement' | 'auth' | 'none';

interface TurnResultCollectorLike {
  onToolStart(toolName: string, toolUseId: string): void;
  onToolEnd(toolName: string, toolUseId: string): void;
  onPhaseChange(phase: string): void;
  onModelCommandResult(result: ModelCommandResult): void;
  onEndTurn(info: any): void;
  onText(text: string): void;
  setContinuation(continuation: Continuation): void;
  getResult(): any;
}

const createNoopTurnResultCollector = (): TurnResultCollectorLike => ({
  onToolStart: () => {},
  onToolEnd: () => {},
  onPhaseChange: () => {},
...
const defaultUserSettingsStore = {
  ...
  getUserDefaultModel: () => undefined,
  getUserDefaultEffort: () => undefined,
  getUserBypassPermission: () => false,
  getUserRating: () => undefined,
  consumePendingRatingChange: () => null,
  getUserSessionTheme: () => undefined,
  getUserCompactThreshold: () => undefined,
};

export interface StreamExecutorProviders {
  parseModelCommandRunResponse?: (result: unknown) => any;
  createTurnResultCollector?: () => TurnResultCollectorLike;
  getChannelDescription?: (client: any, channel: string) => Promise<string>;
  getChannel?: (channel: string) => any;
  fetchClaudeStatus?: () => Promise<any>;
  ...
  getMetricsEmitter?: () => {
    ...
    emitTokenUsage: (...args: any[]) => Promise<unknown> | { catch: (handler: (err: unknown) => unknown) => unknown };
  };
  classifyOneMUnavailable?: (text: string) => OneMUnavailableKind;
  hasOneMSuffix?: (model: string) => boolean;
  isOneMContextUnavailableSignal?: (text: string) => boolean;
  resolveContextWindow?: (modelName?: string) => number;
  stripOneMSuffix?: (model: string) => string;
  interceptToolResults?: (...args: any[]) => void;
  checkAndSchedulePendingCompact?: (args: any) => Promise<unknown>;
  buildCompactionContext?: (snapshot: any) => any;
  snapshotFromSession?: (session: ConversationSession) => any;
  getTokenManager?: () => any;
...
const ONE_M_CONTEXT_UNAVAILABLE_CODE = 'ONE_M_CONTEXT_UNAVAILABLE';
const PRICING_VERSION = '2026-04-17';
const SLACK_BLOCK_KIT_CHANNEL_NAME = 'slack-block-kit';

let streamExecutorProviders: Required<StreamExecutorProviders> = {
  parseModelCommandRunResponse: () => null,
  createTurnResultCollector: createNoopTurnResultCollector,
  getChannelDescription: async () => '',
  getChannel: () => undefined,
  fetchClaudeStatus: async () => null,
...
const isApiLikeError = (error: unknown) => streamExecutorProviders.isApiLikeError(error);
const shouldShowStatusBlock = (status: any) => streamExecutorProviders.shouldShowStatusBlock(status);
const createConversation = (channel: string, threadTs: string, user: string, userName: string) =>
  streamExecutorProviders.createConversation(channel, threadTs, user, userName);
const recordAssistantTurn = (conversationId: string, text: string) =>
...
const isMidThreadMention = (input: { threadTs?: string; mentionTs?: string }) =>
  streamExecutorProviders.isMidThreadMention(input);
const getMetricsEmitter = () => streamExecutorProviders.getMetricsEmitter();
const classifyOneMUnavailable = (text: string) => streamExecutorProviders.classifyOneMUnavailable(text);
const hasOneMSuffix = (model: string) => streamExecutorProviders.hasOneMSuffix(model);
const isOneMContextUnavailableSignal = (text: string) => streamExecutorProviders.isOneMContextUnavailableSignal(text);
const resolveContextWindow = (modelName?: string) => streamExecutorProviders.resolveContextWindow(modelName);
const stripOneMSuffix = (model: string) => streamExecutorProviders.stripOneMSuffix(model);
const interceptToolResults = (...args: any[]) => streamExecutorProviders.interceptToolResults(...args);
const checkAndSchedulePendingCompact = (args: any) => streamExecutorProviders.checkAndSchedulePendingCompact(args);
const buildCompactionContext = (snapshot: any) => streamExecutorProviders.buildCompactionContext(snapshot);
const snapshotFromSession = (session: ConversationSession) => streamExecutorProviders.snapshotFromSession(session);
...
export class StreamExecutor {
  ...
  private handleEnrichmentFailure(
    ...
    // reason. PROJ-4318 / soma-work#933.
...
오디오/비디오 파일만 다운로드가 차단됩니다 — 파일 이름과 메타데이터만 참조하세요.
  ...
  async execute(params: StreamExecuteParams): Promise<ExecuteResult> {
    ...
    const usageBeforePromise: Promise<UsagePercentSnapshot | null> = activeSlotSnapshot
      ? getTokenManager()
          ...
          .then((snap: UsageSnapshot | null | undefined) => toUsagePercentSnapshot(snap))
          .catch(() => null)
    ...
    const turnCollector = streamExecutorProviders.createTurnResultCollector();
      ...
      const streamCallbacks: StreamCallbacks = {
        ...
        onToolResult: async (toolResults, ctx) => {
          ...
          interceptToolResults(
            ...
            (_sessionKey: string, prNumber: number, linesAdded: number, linesDeleted: number) => {
        ...
        onSessionLinksDetected: async (links) => {
          this.deps.claudeHandler.setSessionLinks(channel, threadTs, links as any);
  ...
  private async handleError(
    ...
    if (shouldNotifyException && this.deps.turnNotifier) {
      ...
      // (PROJ-4318 / soma-work#933). Coalesce through `.message` → `.code` →
  ...
  private async tryRotateToken(error: any, activeSlotAtQueryStart: ActiveTokenInfo | null): Promise<void> {
    ...
    // yields a future wall-clock we use it as the cooldown duration; otherwise
    // we fall back to the 60-minute default. A parsed reset that is already in
    // the past is treated as the fallback case (it'd otherwise be clamped to
    // a 1-minute cooldown, which produces a useless near-immediate retry).
      ...
      if (parsed.commandId === 'MANAGE_SKILL') {
        ...
        if (payload.ok && payload.mutated?.kind === 'skill') {
          try {
            ...
            // Non-fatal — the cache will self-heal on the next reset point.
            this.logger.debug('MANAGE_SKILL invalidation hook failed', {
              sessionKey: context.sessionKey,
  ...
  private async supersedePriorPendingChoice(
    ...
    if (prior.kind === 'multi' && prior.formIds.length > 0) {
      ...
      await Promise.allSettled(
        prior.formIds.map(async (formId: string) => {
  ...
  private async sendCommandChoiceFallback(question: UserChoice | UserChoices, context: StreamContext): Promise<void> {
    ...
    if (question.type === 'user_choices') {
      const lines = [
        ...
        '',
        ...question.questions.map((entry: any, index: number) => {

...
import { type LegacyCommandRouter, parseTopic, ZRouter } from './z/router';
...
export interface SessionExpiryCallbacks {
  onWarning: (
    session: EventConversationSession,
    timeRemaining: number,
    existingMessageTs?: string,
  ) => Promise<string | undefined>;
  onSleep: (session: EventConversationSession) => Promise<void>;
  onExpiry: (session: EventConversationSession) => Promise<void>;
}

export interface EventSessionLink {
  url?: string;
  label?: string;
}

export interface EventConversationSession {
  channelId: string;
  threadTs?: string;
  threadRootTs?: string;
  sessionId?: string;
...
export interface ClaudeSessionEventRouter {
  getSession(channel: string, threadTs?: string): EventConversationSession | undefined;
  findSessionBySourceThread(channel: string, threadTs?: string): EventConversationSession | undefined;
  setExpiryCallbacks(callbacks: SessionExpiryCallbacks): void;
  cleanupInactiveSessions(): Promise<void>;
  broadcastSessionUpdate(): void;
}

export interface SessionUiEventManager {
  formatUserSessionsBlocks(
    userId: string,
    options?: { showControls?: boolean },
  ...
  handleSessionSleep(session: EventConversationSession): Promise<void>;
  handleSessionExpiry(session: EventConversationSession): Promise<void>;
}

export interface EventRouterProviders {
  createLegacyCommandRouter?: (deps: unknown) => LegacyCommandRouter;
  ...
  downloadSkillUploadFile?: (file: FileDescriptor) => Promise<DownloadFileResult>;
...
let eventRouterProviders: Required<EventRouterProviders> = {
  createLegacyCommandRouter: () => {
    throw new Error('EventRouter legacy command router provider is not configured.');
  },
  markMigrationHintShown: async () => false,
  hasMigrationHintShown: () => false,
  registerChannel: async () => {},
  unregisterChannel: () => {},
  getBaseDirectory: () => undefined,
  downloadSkillUploadFile: async () => ({ ok: false, error: 'Skill upload downloader is not configured.' }),
...
export class EventRouter {
  ...
  constructor(
    ...
    if (deps.commandDeps) {
      this.commandRouter = eventRouterProviders.createLegacyCommandRouter(deps.commandDeps);
      this.zRouter = new ZRouter({
        legacyRouter: this.commandRouter,
        tombstoneStore: {
          markMigrationHintShown: (uid) => eventRouterProviders.markMigrationHintShown(uid),
          hasMigrationHintShown: (uid) => eventRouterProviders.hasMigrationHintShown(uid),
        },
      });
    }
  }
   ...
   */
  private async maybeConsumeSkillUpload(messageEvent: any, session: EventConversationSession): Promise<boolean> {
    ...
    const outcome = await consumePendingSkillUpload({
      ...
      files,
      deps: {
        readCurrentContent: eventRouterProviders.readCurrentSkillContent,
        hashContent: eventRouterProviders.hashSkillContent,
        applyUpdate: eventRouterProviders.applySkillUpdate,
        downloadFile: eventRouterProviders.downloadSkillUploadFile,
      },
    });
  ...
  private setupMemberJoinHandler(): void {
    this.app.event('member_joined_channel', async ({ event, say }) => {
      ...
      if (event.user === botUserId) {
        ...
        await eventRouterProviders.registerChannel(this.deps.slackApi.getClient(), event.channel);
    ...
    this.app.event('member_left_channel' as any, async ({ event }: any) => {
      ...
      if (event.user === botUserId) {
        ...
        eventRouterProviders.unregisterChannel(event.channel);
      }
    });
  }
  ...
  private async handleChannelJoin(channelId: string, say: SayFn): Promise<void> {
    try {
      ...
      const baseDirectory = eventRouterProviders.getBaseDirectory();
      if (baseDirectory) {
        welcomeMessage += `You can use:\n`;
        welcomeMessage += `• \`cwd project-name\` (relative to base directory: \`${baseDirectory}\`)\n`;

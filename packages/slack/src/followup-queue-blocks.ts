...
/**
 * ONE queued item as ONE thread message (A39) — the surface that replaced the
 * panel's Queue section.
 *
 * The user asked to read the queue where they typed, with the two controls in
 * reach ("thread안에 유저가 메세지 쳤을때마다 출력해줘 즉시 send now / cancel 할수
 * 있도록"), so the host posts this right under the message it parked. Two blocks:
 * the compact line the panel already used ({@link compactItemLine} — same wording
 * in both places by construction) and an actions row carrying `Send now` and
 * `Cancel` as BUTTONS. The overflow menu is not reused here: a one-item message
 * has room for the real labels, and an overflow gives no visual feedback at all.
 *
 ...
 *     on `Send now` alone (A12/A28: it targets the LIVE turn, Cancel does not);
 ...
 *
 * `index` defaults to the item's `seq` (its FIFO position, what the panel
 * showed); the `queue` command passes a 1-based list position instead.
 */
export function buildFollowupItemMessage(item: FollowupItem, turnEpoch: number, index?: number): FollowupItemMessage {
  const seq = index ?? item.seq;
  const sendNow = itemButton(item, FOLLOWUP_SEND_NOW_ACTION_ID, FOLLOWUP_SEND_NOW_LABEL, {
    style: 'primary',
    turnEpoch,
  });
  ...
  const elements = [sendNow, cancel].filter((button): button is Record<string, unknown> => button !== null);

  const blocks: unknown[] = [
    {
      type: 'section',
      text: { type: 'mrkdwn', text: compactItemLine(item, seq, elements.length === 0), verbatim: true },
    },
  ];
  if (elements.length > 0) blocks.push({ type: 'actions', elements });

  return {
    text: `${FOLLOWUP_QUEUE_TITLE} ${seq}. ${compactPreview(item)} · ${escapeSlackMrkdwn(compactStateLabel(item))}`,
    blocks,
  };
}

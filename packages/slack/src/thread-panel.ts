...
export interface ThreadPanelClaudeHandler {
  getSessionByKey(sessionKey: string): ConversationSession | undefined;
  getSessionKey(channelId: string, threadTs: string): string | undefined;
}

export interface ThreadPanelSessionRegistry {
  persistAndBroadcast(sessionKey: string): void;
}

export interface ThreadPanelCompletionChannel {
  send(event: TurnCompletionEvent): Promise<void>;
}

export interface ThreadPanelDeps {
  slackApi: SlackApiHelper;
  claudeHandler: ThreadPanelClaudeHandler;
   ...
   */
  sessionRegistry?: ThreadPanelSessionRegistry;
  /**

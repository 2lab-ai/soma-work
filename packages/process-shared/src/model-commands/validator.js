...
exports.LEGACY_RECOMMENDED_SUFFIX_RE = void 0;
exports.validateModelCommandRunArgs = validateModelCommandRunArgs;
exports.checkAskUserQuestionQuality = checkAskUserQuestionQuality;
const RESOURCE_TYPES = ['issue', 'pr', 'doc'];
...
const ASK_USER_QUESTION_ALLOWED_TYPES = ['user_choice', 'user_choice_group'];
...
function validateModelCommandRunArgs(args) {
    if (!isRecord(args)) {
...
function buildSaveContextFallbackParams(args) {
    const fallback = {};
    if ('result' in args) {
...
function parseUpdateSessionRequest(raw) {
    if (!isRecord(raw)) {
...
function parseSessionOperation(raw) {
    if (!isRecord(raw)) {
    ...
    if (action === 'add') {
        const link = normalizeLink(raw.link, resourceType);
        if (!link) {
...
function parseAskUserQuestionParams(raw) {
    if (!isRecord(raw)) {
 ...
 */
function checkAskUserQuestionQuality(params) {
    const warnings = [];
 ...
 */
exports.LEGACY_RECOMMENDED_SUFFIX_RE = /\(Recommended(?:\s*·\s*\d+\/\d+)?\)\s*$/i;
...
function collectUserChoiceWarnings(payload, warnings) {
    ...
    if (questionTrimmed.length >= 1 && !TIER_PREFIX_RE.test(questionText)) {
        warnings.push('question missing tier prefix — expected [tiny|small|medium|large|xlarge] (optionally with ~N lines)');
    }
...
function collectUserChoicesWarnings(payload, warnings) {
    ...
    const titleHasPrefix = titleTrimmed.length >= 1 && TIER_PREFIX_RE.test(title);
    if (!titleHasPrefix) {
        warnings.push('question missing tier prefix — expected [tiny|small|medium|large|xlarge] (optionally with ~N lines)');
    }
...
function pushContextWarnings(context, prefix, warnings) {
...
function pushOptionCountWarnings(choices, prefix, warnings) {
...
function pushRecommendedWarnings(choices, prefix, warnings, recommendedChoiceId) {
    ...
    if (typeof recommendedChoiceId === 'string' &&
...
function pushForbiddenLabelWarnings(choices, prefix, warnings) {
    for (const choice of choices) {
        if (typeof choice.label !== 'string')
            continue;
        const stripped = choice.label.replace(RECOMMENDED_MARKER_RE, '');
        if (isForbiddenMetaLabel(stripped)) {
            warnings.push(`${prefix}option [${choice.id}] label '${choice.label}' is a meta/approval verb — use actionable domain-specific verb phrase`);
        }
    }
}
function isForbiddenMetaLabel(label) {
...
function parseSaveContextResultParams(raw) {
    if (!isRecord(raw)) {
...
function parseContinueSessionParams(raw) {
    if (!isRecord(raw)) {
    ...
    if (forceWorkflow !== undefined) {
        if (typeof forceWorkflow !== 'string' || !WORKFLOW_TYPES.includes(forceWorkflow)) {
    ...
    return {
        ...
        value: {
            ...
            forceWorkflow: forceWorkflow,
        },
    };
}
function normalizeSaveContextResultFromVariants(raw) {
    const candidates = [];
...
function invalidAskUserQuestionArgs(raw, reason) {
...
function normalizeLink(raw, resourceType) {
    if (!isRecord(raw)) {
        return null;
    }
    if (typeof raw.url !== 'string' || raw.url.trim() === '') {
...
function normalizeChoicePayload(raw) {
    if (!isRecord(raw)) {
        return null;
    }
    ...
    if (raw.type === 'user_choices') {
        const questions = Array.isArray(raw.questions)
            ...
            : [];
        const compact = questions.filter((question) => !!question);
        if (compact.length === 0) {
            return null;
        }
        return {
...
function normalizeUserChoiceQuestion(raw, index) {
    if (!isRecord(raw)) {
        return null;
    }
 ...
 */
function resolveRecommendedChoiceId(explicitId, options) {
    ...
    const legacy = options.find((o) => exports.LEGACY_RECOMMENDED_SUFFIX_RE.test(o.label));
    return legacy?.id;
}
...
function normalizeSaveContextResult(raw) {
    const result = {};

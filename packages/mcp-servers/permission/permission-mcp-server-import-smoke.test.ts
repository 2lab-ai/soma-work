...
describe('permission-mcp-server import smoke', () => {
  ...
  it('resolves SlackPermissionMessenger from process-shared/permission and constructs the server', async () => {
  ...
  it('exposes PermissionRuleSummary-shaped overridableRules path on buildRequestBlocks', async () => {
    const { SlackPermissionMessenger } = await import('@soma/process-shared/permission/slack-messenger.js');
